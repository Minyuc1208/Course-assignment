...
function updateGrades() {
    ...
    // HOMEWORK STEP 5 GOES HERE
     var studentPercentageColumn =gradebookData[i].studentRow.firstChild.nextSibling.nextSibling;
      studentPercentageColumn.textContent = studentPercentage.toFixed(1) + "%";
    }
}
...
function promptForUpdateGrade(){
    var studentID = 0;
    var assignmentID = 0;
    var point = 0;
    
    console.log('gradebokData.length='+gradebookData.length);
    do{
        studentID = Number(prompt("Please enter your student ID number, number MUSTBE between 0 and " + (gradebookData.length-1)));
} while ((studentID < 0) || (studentID >= gradebookData.length ));
    
    
    do{
        assignmentID = Number(prompt("Please enter your assignment ID number, number MUSTBE between 0 and " + (assignmentID.length-1)));
    }while((assignmentID<0)||(assignmentID>=assignmentID.length));

    
    do{
        point= Number(prompt("Please enter the grade, number MUSTBE between 0 and " + assignmentData[assignmentID].totalPointValue));
    }while((point<0)||(point>assignmentData[assignmentID].totalPointValue));
    
    updateStudentGrade(studentID,assignmentID,point);
}
...
updateGradeButton.addEventListener('click',promptForUpdateGrade,false);
